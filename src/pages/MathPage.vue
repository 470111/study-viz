...
const minutesByDay = computed<Record<string, number>>(() => {
  ...
  for (const s of store.sessions.filter((x) => x.subjectId === 'math')) {
...
const last7 = computed(() => {
  // naive: take last 7 days by sorting keys (good enough for now)
...
<template>
  <SectHeader sect="天算宗" subtitle="数学" subject-id="math" />
  <div class="stack">
    <section class="panel">
      <div class="p-head">
    ...
    <WeeklyReportCard subject-id="math" />
    <StageCard subject-id="math" />
    <TimerCard subject-id="math" />
    <CheckinCard subject-id="math" />
    <InsightCard subject-id="math" />
    <WrongbookCard subject-id="math" />
  </div>
</template>
